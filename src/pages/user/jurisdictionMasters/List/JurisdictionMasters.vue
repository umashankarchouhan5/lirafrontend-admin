<template>
    <FormWrapper :is-loading="isLoading">
        <PageTitle title="Jurisdiction Masters" slot="title" />
...
<script>
    ...
    import { getProcessTypes, getProcesses, fetchAllJurisdiction } from '../api'
    export default {
        ...
        methods: {
            ...
            getDataByFilter() {
                ...
                fetchAllJurisdiction(data)
                    .then(this.handleResponse)
                    .catch(this.handleError)
            },
            handleResponse(response) {
                this.hideLoader();
                this.Jurisdics = response.data;
            ...
            pushTo(mode) {
                if (this.validateSelectedLength(1)) {
                    let juricId = this.selectedRows[0].id; // allow only the first row
                    if( mode== 'edit' ){ // edit is clicked
                        this.$router.push({ path: `/jurisdictionMasters/edit/${juricId}` })
                    }
                }
            },
            addNew() {
                this.$router.push({ path: `/jurisdictionMasters/add/` })
            },
        }
    }
</script>
