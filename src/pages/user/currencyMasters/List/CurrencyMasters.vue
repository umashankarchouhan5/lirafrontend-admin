<template>
    <FormWrapper :is-loading="isLoading">
        <PageTitle title="Currency Masters" slot="title" />
        ...
        methods: {
            ...
            pushTo(mode) {
                if (this.validateSelectedLength(1)) {
                    let code = this.selectedRows[0].code; // allow only the first row
                    if( mode== 'edit' ){ // edit is clicked
                        this.$router.push({ path: `/currencyMasters/edit/${code}` })
                    }
                }
            },
            addNew() {
                this.$router.push({ path: `/currencyMasters/add/` })
            },
        }
    }
</script>
