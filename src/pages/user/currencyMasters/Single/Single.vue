    ...
    <ValidationObserver ref="observer" v-slot="{ passes }">
        <FormWrapper :is-loading="isLoading" >
            ...
            <CenterBlock :width="700">
                <FormRow>
                    <div class="col-sm-6" >
                        <InputText readonly v-model="form.code"  rules="required" label="Code" maxlength="100" v-if="mode=='edit'"/>
                        <InputText v-model="form.code"  rules="required" label="Code" maxlength="100" v-else/>
                    </div>
                    <div class="col-sm-6" >
                        <InputText v-model="form.Name"  label="Name" maxlength="100" />
                    </div>
                </FormRow>
                <FormRow>
...
<script>
    ...
    import { addCurrency, getCurrencyDataByCode } from "../api.js";
    ...
    export default {
        ...
        created() {
                ...
                if(typeof this.code!='undefined' && this.code!=null){
                    this.mode = 'edit';
                    this.getCurrencyDataByCode(this.code);
                }
        },
        computed: {
        ...
        methods: {
            backToList(){
                this.$router.push('/currencyMasters');
            },
            formSubmit() {
                this.showLoader();
                if(typeof this.mode!="undefined" && this.mode!=null){
                    const data = {Name:this.form.Name, code:this.code, isEdit:true}
                    addCurrency(data).then(this.handleSuccessResponse)
                } else {
                    addCurrency(this.form).then(this.handleSuccessResponse,this.handleErrorResponse)   
                }
            },
            ...
            getCurrencyDataByCode(code){
                this.showLoader();
                getCurrencyDataByCode({code:code}).then(this.handleResponse)
            },
