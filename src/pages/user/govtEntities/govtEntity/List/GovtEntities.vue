<template>
    <FormWrapper :is-loading="isLoading">
        <PageTitle title="Govt Entities" slot="title"  />
...
<script>
    ...
    import { getGovtEntitiesList } from '../api'
    ...
    export default {
        ...
        methods: {
            addEntity(){
                this.$router.push('/govtEntityCreation');
            },
            editEntity(){
                 if (this.validateSelectedLength(1)) {
                    const row = this.selectedRows[0];
                    this.$router.push(`govtEntityUpdate/${row.id}`);
                }
            },
            entityUsers(){
                if (this.validateSelectedLength(1)) {
                    const row = this.selectedRows[0];
                    this.$router.push(`govtEntityUsers/${row.id}`);
                }
            },
            selectChange(data) {
