<template>
    <FormRow>
        ...
        <div class="col-sm-2" v-if="value.AnswerYN === 1 || value.FieldType.toLowerCase() === 'checkbox'">
            <div :class="$style.selectors" >
                <FormCheckbox :disabled="readonly || disabled" :rules="{required: value.isMandatory === 1 && disabled === false ? false: true}"  name="NA Box" :trueValue="1"  :falseValue="0" v-model="value.ValueTextArea" ></FormCheckbox>
        ...
        <div v-if="value.DocumentWhenAnswerYN  === null || value.DocumentWhenAnswerYN === value.ValueTextArea" class="col-sm-12">
            <div :class="$style.file" v-if="value.AllowDocumentUpload === 1 || (value.AnswerYN === 0 || (value.AnswerYN === 1 && value.ValueTextArea !== null))">
                <InputFile label="Select File (pdf)" :readonly="readonly || disabled" :disabled="readonly || disabled" v-if="this.value.DocumentPath === null" @onChange="uploadFile" :rules="{required:disabled ? false: true,docCheck:'pdf'}" />
