    ...
    <ValidationProvider :vid="vid" :rules="rules"  :name="name || label" mode="passive" v-slot="{ errors }">
        <div :class="cssClass">
            <input
                ...
                min="0"
                :style="getStyle"
                :autocomplete="passwordOff? 'new-password' : 'disabled'"
            />
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            handleKeyUp(){
                this.innerValue = this.type !== 'number' ? this.innerValue : Number(this.innerValue);
                this.innerValue = this.noApostrophe ? this.innerValue.replace(/'+/g,'') : this.innerValue;
                this.$emit("keyup");
            },
            handleKeyDown(e) {
                if(this.type.toLowerCase() == 'number' && e.keyCode == 69) e.preventDefault();
            }
        }
    };
</script>
